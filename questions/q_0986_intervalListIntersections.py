class Solution:
    ...
    `secondList[j] = [startj, endj]`. Each list of intervals is pairwise
    ...
    **Constraints:**
    ...
    def intervalIntersection(
        ...
        indices = {0: 0, 1: 0}
        result = []
        while indices[0] < len(firstList) and indices[1] < len(secondList):
            start = max(firstList[indices[0]][0], secondList[indices[1]][0])
            end = min(firstList[indices[0]][1], secondList[indices[1]][1])
            if start <= end:
                result.append([start, end])

            if firstList[indices[0]][1] < secondList[indices[1]][1]:
                indices[0] += 1
            else:
                indices[1] += 1
        return result
